...
#ifndef Coordinates_TL_class
#define Coordinates_TL_class
class Coordinates_TL {

public:
    Coordinates_TL(int lx, int ly, int n_orbs)
...
/*  
 * ***********
 *  Functions in Class Coordinates_TL ------
 *  ***********
*/  

int Coordinates_TL::indx_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_TL.h:x-coordinate of lattice excede limit");}
    return indx_basiswise_[i];
    // ----------
}


int Coordinates_TL::indy_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_TL.h:y-coordinate of lattice excede limit");}
    return indy_basiswise_[i];
    // ----------
}

int Coordinates_TL::indx_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_TL.h:x-coordinate of lattice excede limit");}
    return indx_cellwise_[i];
    // ----------
}


int Coordinates_TL::indy_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_TL.h:y-coordinate of lattice excede limit");}
    return indy_cellwise_[i];
    // ----------
}

int Coordinates_TL::Nbasis(int x, int y, int orb){
    if(!( (x<lx_&& y<ly_) &&  orb<n_orbs_)){perror("Coordinates_TL.h:ith-sitelabel of lattice excede limit");}
