...
#ifndef Coordinates_LL_class
#define Coordinates_LL_class
class Coordinates_LL {

public:
    Coordinates_LL(int lx, int ly, int n_orbs)
...
/*  
 * ***********
 *  Functions in Class Coordinates_LL ------
 *  ***********
*/  

int Coordinates_LL::indx_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_LL.h:x-coordinate of lattice excede limit");}
    return indx_basiswise_[i];
    // ----------
}


int Coordinates_LL::indy_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_LL.h:y-coordinate of lattice excede limit");}
    return indy_basiswise_[i];
    // ----------
}

int Coordinates_LL::indx_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_LL.h:x-coordinate of lattice excede limit");}
    return indx_cellwise_[i];
    // ----------
}


int Coordinates_LL::indy_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_LL.h:y-coordinate of lattice excede limit");}
    return indy_cellwise_[i];
    // ----------
}

int Coordinates_LL::Nbasis(int x, int y, int orb){
    if(!( (x<lx_&& y<ly_) &&  orb<n_orbs_)){perror("Coordinates_LL.h:ith-sitelabel of lattice excede limit");}
    return Nbasis_[x][y][orb];
    // ----------
}


int Coordinates_LL::Ncell(int x, int y){
    if(!(x<lx_&& y<ly_)){perror("Coordinates_LL.h:ith-sitelabel of lattice excede limit");}
    return Ncell_(x,y);
    // ----------
}


int Coordinates_LL::neigh(int cell, int wneigh){
    if(cell> (lx_*ly_)-1 || wneigh>=8){perror("Coordinates_LL.h:getneigh -> ifstatement-1");}
    return neigh_(cell,wneigh);
} // ----------


void Coordinates_LL::Numbering(){
...
int Coordinates_LL::getneigh(int site,int wneigh){
    if(site>ncells_-1 || wneigh>7){perror("Coordinates_LL.h:getneigh -> ifstatement-1");}
