...
#ifndef Kspace_calculation_LL_class
#define Kspace_calculation_LL_class

extern "C" void   zheev_(char *,char *,int *,std::complex<double> *, int *, double *,
                         std::complex<double> *,int *, double *, int *);

class Kspace_calculation_LL
{
public:
    Kspace_calculation_LL(Parameters_LL &Parameters__, Coordinates_LL &Coordinates__, mt19937_64& Generator1__ )
    ...
    void Create_Kspace_Spectrum_in_fourtimes_brillouin_zone();
    void Calculate_ChernNumbers_in_fourtimes_brillouin_zone();
...
void Kspace_calculation_LL::Calculate_ChernNumbers_in_double_brillouin_zone(){
...
void Kspace_calculation_LL::Calculate_ChernNumbers_in_ntimes_brillouin_zone(int N_){


    //cout<<"here"<<endl;
    Matrix<complex<double>> F_mat; //F1, F2, F3, F4, F5;
    F_mat.resize(6, N_*N_*lx_*ly_);
    //cout<<"here"<<endl;
    ...
    for (int band = 0; band < 6; band++)
    {
        ...
        F_bands[band] = 0.0;
        for (int nx = 0; nx < N_*lx_; nx++)
        {
            for (int ny = 0; ny < N_*ly_; ny++)
            {
                ...
                Ux_k = 0;
                n_left = n;
                nx_right = (nx + 1) % (N_*lx_);
                ny_right = ny;
                n_right = nx_right + N_*lx_*ny_right;
                for (int comp = 0; comp < 6; comp++)
                {
                    Ux_k +=
                            conj(Eigvectors_[6*n_left +  band][comp]) *
                ...
                nx_left = (nx + 1) % (N_*lx_);
                ny_left = ny;
                n_left = nx_left + N_*lx_*ny_left;
                nx_right = nx_left;
                ny_right = (ny_left + 1) % (N_*ly_);
                n_right = nx_right + N_*lx_*ny_right;
                for (int comp = 0; comp < 6; comp++)
                {
                    Uy_kpx +=
                ...
                if(ny==N_*ly_-1){//For pm3d corners2color c1
                    fl_Fk_out<<nx<<"  "<<ny<<"  "<<F_mat(band, n).real()<<"  "<<F_mat(band, n).imag()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).real()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).imag()<<endl;
                }
            }
            if(nx==N_*lx_-1){//For pm3d corners2color c1
                fl_Fk_out<<endl;
                for(int ny_=0;ny_<N_*ly_;ny_++){
                    int n_ = nx + N_*lx_* ny_;
                    fl_Fk_out<<nx<<"  "<<ny_<<"  "<<F_mat(band, n_).real()<<"  "<<F_mat(band, n_).imag()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).real()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).imag()<<endl;
                }
...
void Kspace_calculation_LL::Calculate_ChernNumbers_in_fourtimes_brillouin_zone(){


    //cout<<"here"<<endl;
    Matrix<complex<double>> F_mat; //F1, F2, F3, F4, F5;
    F_mat.resize(6, 16*lx_*ly_);
    //cout<<"here"<<endl;
    ...
    {
        //cout<<"here"<<endl;
        string file_Fk="Fk_band"+to_string(band)+"_fourtimes_brillouin_zone.txt";
        ofstream fl_Fk_out(file_Fk.c_str());
        fl_Fk_out<<"#nx  ny  tilde_F(nx,ny).real()  tilde_F(nx,ny).imag()  ArgofLog.real()  ArgofLog.imag()"<<endl;
        fl_Fk_out<<"#Extra momentum point for pm3d corners2color c1"<<endl;

        F_bands[band] = 0.0;
        for (int nx = 0; nx < 4*lx_; nx++)
        {
            for (int ny = 0; ny < 4*ly_; ny++)
            {
                ...
                nx_right = (nx + 1) % (4*lx_);
                ny_right = ny;
                n_right = nx_right + 4*lx_*ny_right;
                for (int comp = 0; comp < 6; comp++)
                {
                    Ux_k +=
                            conj(Eigvectors_[6*n_left +  band][comp]) *
                ...
                nx_left = (nx + 1) % (4*lx_);
                ny_left = ny;
                n_left = nx_left + 4*lx_*ny_left;
                nx_right = nx_left;
                ny_right = (ny_left + 1) % (4*ly_);
                n_right = nx_right + 4*lx_*ny_right;
                for (int comp = 0; comp < 6; comp++)
                {
                    Uy_kpx +=
                ...
                if(ny==4*ly_-1){//For pm3d corners2color c1
                    fl_Fk_out<<nx<<"  "<<ny<<"  "<<F_mat(band, n).real()<<"  "<<F_mat(band, n).imag()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).real()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).imag()<<endl;
                }
            }
            if(nx==4*lx_-1){//For pm3d corners2color c1
                fl_Fk_out<<endl;
                for(int ny_=0;ny_<4*ly_;ny_++){
                    int n_ = nx + 4*lx_* ny_;
                    fl_Fk_out<<nx<<"  "<<ny_<<"  "<<F_mat(band, n_).real()<<"  "<<F_mat(band, n_).imag()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).real()<<
                               "  "<<(Ux_k*Uy_kpx*conj(Ux_kpy)*conj(Uy_k)).imag()<<endl;
        ...
        Chern_num[band] = (-1.0 * iota_complex / (2 * PI*16)) * F_bands[band];
        // Chern_num_orgnl[band] = (-1.0 * iota_complex / (2 * PI)) * F_bands_orgnl[band];
        fl_Fk_out<<"#Chern no*2pi*Iota= "<<F_bands[band].real()<<"  "<<F_bands[band].imag()<<endl;
        cout << "tilde Chern number [" << band << "] = " << Chern_num[band].real() << "        " << Chern_num[band].imag() << endl;
        //  cout << "Chern number [" << band << "] = " << Chern_num_orgnl[band].real() << " " << Chern_num_orgnl[band].imag() << endl;
...
void Kspace_calculation_LL::Calculate_ChernNumbers(){
...
void Kspace_calculation_LL::Get_Bands(){
...
void Kspace_calculation_LL::Get_Energies(){
...
void Kspace_calculation_LL::Get_new_OPs_and_error(){
...
double Kspace_calculation_LL::chemicalpotential(double Particles){
...
double Kspace_calculation_LL::random1(){

    return dis1_(Generator1_);

}

void Kspace_calculation_LL::Initialize()
{
...
void Kspace_calculation_LL::Arranging_spectrum(){
...
void Kspace_calculation_LL::Create_Kspace_Spectrum_in_double_brillouin_zone(){
    ...
    for(int k1=0;k1<2*lx_;k1++){
        ...
        for(int k2=0;k2<2*ly_;k2++){
            ...
            //Hoppings
            for(int spin=0;spin<2;spin++){
                  Ham_(spin + 2*1, spin + 2*0) += Parameters_.hopping_intracell*2.0*cos(Kx_values[k_index]*0.5);
                  Ham_(spin + 2*2, spin + 2*0) += Parameters_.hopping_intracell*2.0*cos(Ky_values[k_index]*0.5);
            }

            //R-SOC
            Ham_(0 + 2*1,1 + 2*0) += -2.0*iota_complex*Parameters_.lambda_RSOC*sin(Kx_values[k_index]*0.5);
            Ham_(1 + 2*1,0 + 2*0) += 2.0*iota_complex*Parameters_.lambda_RSOC*sin(Kx_values[k_index]*0.5);
            Ham_(0 + 2*2,1 + 2*0) += -2.0*Parameters_.lambda_RSOC*sin(Ky_values[k_index]*0.5);
            Ham_(1 + 2*2,0 + 2*0) += -2.0*Parameters_.lambda_RSOC*sin(Ky_values[k_index]*0.5);
...
void Kspace_calculation_LL::Create_Kspace_Spectrum_in_fourtimes_brillouin_zone(){

    Eigvectors_.resize(ncells_*6*16);
    Eigenvalues_.resize(ncells_*6*16);
    for(int i=0;i<ncells_*6*16;i++){
        Eigvectors_[i].resize(6); //Eigenvector number
    }

    Kx_values.resize(ncells_*16);
    Ky_values.resize(ncells_*16);


    int k_index;
    double k1x_val, k2x_val, k1y_val, k2y_val;
    for(int k1=0;k1<4*lx_;k1++){
        k1x_val = (2.0*PI*k1)/(1.0*lx_);
        k1y_val = 0.0;

        for(int k2=0;k2<4*ly_;k2++){
            k2x_val = 0.0;
            k2y_val = ((2.0*PI*k2)/(1.0*ly_));
...
void Kspace_calculation_LL::Create_Kspace_Spectrum_in_ntimes_brillouin_zone(int N_){

    Eigvectors_.resize(ncells_*6*N_*N_);
    Eigenvalues_.resize(ncells_*6*N_*N_);
    for(int i=0;i<ncells_*6*N_*N_;i++){
        Eigvectors_[i].resize(6); //Eigenvector number
    }
    ...
    int k_index;
    double k1x_val, k2x_val, k1y_val, k2y_val;
    for(int k1=0;k1<N_*lx_;k1++){
        k1x_val = (2.0*PI*k1)/(1.0*lx_);
        k1y_val = 0.0;

        for(int k2=0;k2<N_*ly_;k2++){
            k2x_val = 0.0;
            k2y_val = ((2.0*PI*k2)/(1.0*ly_));

            k_index = k1 + N_*lx_*k2;
            Kx_values[k_index]=k1x_val + k2x_val;
            Ky_values[k_index]=k1y_val + k2y_val;
...
void Kspace_calculation_LL::Create_Kspace_Spectrum(){
...
void Kspace_calculation_LL::SelfConsistency(){
    ...
    cout<<"Chern numbers in four times of brillouin zone"<<endl;
    Create_Kspace_Spectrum_in_fourtimes_brillouin_zone();
    cout<<"Spectrum created"<<endl;
    Calculate_ChernNumbers_in_fourtimes_brillouin_zone();


    int Ntimes=12;
    cout<<"Chern numbers in "<< Ntimes <<" times of brillouin zone"<<endl;
...
void Kspace_calculation_LL::Diagonalize(char option){
