...
#ifndef Coordinates_HC_MO_class
#define Coordinates_HC_MO_class
class Coordinates_HC_MO {

public:
    Coordinates_HC_MO(int lx, int ly, int n_atoms, int n_orbs)
...
/*  
 * ***********
 *  Functions in Class Coordinates_HC_MO ------
 *  ***********
*/  

int Coordinates_HC_MO::indx_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_HC_MO.h:x-coordinate of lattice excede limit");}
    return indx_basiswise_[i];
    // ----------
}


int Coordinates_HC_MO::indy_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_HC_MO.h:y-coordinate of lattice excede limit");}
    return indy_basiswise_[i];
    // ----------
}

int Coordinates_HC_MO::indx_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_HC_MO.h:x-coordinate of lattice excede limit");}
    return indx_cellwise_[i];
    // ----------
}


int Coordinates_HC_MO::indy_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_HC_MO.h:y-coordinate of lattice excede limit");}
    return indy_cellwise_[i];
    // ----------
}

int Coordinates_HC_MO::Nbasis(int x, int y, int gamma){
    if(!( (x<lx_&& y<ly_) &&  gamma<4 )){perror("Coordinates_HC_MO.h:ith-sitelabel of lattice excede limit");}
    return Nbasis_[x][y][gamma];
    // ----------
}


int Coordinates_HC_MO::Ncell(int x, int y){
    if(!(x<lx_&& y<ly_)){
        cout<<x<<"   "<<y<<endl;
        perror("Coordinates_HC_MO.h:ith-sitelabel of lattice excede limit : Ncell(int x, int y) ");
    }
    return Ncell_(x,y);
    // ----------
}
...
int Coordinates_HC_MO::neigh(int cell, int wneigh){
    if(cell> (lx_*ly_)-1 || wneigh>=17){
        cout<<cell<<"  "<<wneigh<<endl;
        perror("Coordinates_HC_MO.h:getneigh -> ifstatement-10");}
    return neigh_(cell,wneigh);
} // ----------


void Coordinates_HC_MO::Numbering(){
...
int Coordinates_HC_MO::getneigh(int site,int wneigh){
    if(site>ncells_-1 || wneigh>17){perror("Coordinates_HC_MO.h:getneigh -> ifstatement-1");}
