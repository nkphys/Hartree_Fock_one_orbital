...
#ifndef Connections_HC_MO_class
#define Connections_HC_MO_class


class Connections_HC_MO
{
public:
Connections_HC_MO(Parameters_HC_MO &Parameters__, Coordinates_HC_MO &Coordinates__)
...
Coordinates_HC_MO &Coordinates_;
...
void Connections_HC_MO::Get_minimum_distance_direction(int l,int gamma_l, int m, int gamma_m, int &r1_, int &r2_, double &dis_min){
...
void Connections_HC_MO::Initialize()
{
...
void Connections_HC_MO::Print_Ansatz_LocalDen_CDW(){
...
void Connections_HC_MO::InteractionsCreate()
{
...
for(l=0;l<nsites_;l++){
    ...
    for(int orb0=0;orb0<n_orbs_;orb0++){
        for(int orb1=0;orb1<n_orbs_;orb1++){
    a = Coordinates_.Nbasis(l1_pos, l2_pos, atom0+2*orb0); //assuming orb=0
    b = Coordinates_.Nbasis(l1_pos, l2_pos, atom1+2*orb1);

    
        Hint_(a,b) = complex<double>(1.0 * Parameters_.U0_interatom, 0.0);
        Hint_(b,a) = Hint_(a,b);
...
for (l = 0; l < nsites_; l++)
{
    ...
    for(int neigh=0;neigh<U_neighs.size();neigh++){
        m = Coordinates_.getneigh(l, U_neighs[neigh]); 
        ...
        if( (!Coordinates_.HIT_X_BC) || Parameters_.PBC_X){
            ...
            if( (!Coordinates_.HIT_Y_BC) || Parameters_.PBC_Y){
                    ...
                    for (int atom2 = 0; atom2 < n_atoms_; atom2++)
                {
                    ...
                    {
                        a = Coordinates_.Nbasis(l1_pos, l2_pos, atom1 + 2*orb1);
                        b = Coordinates_.Nbasis(m1_pos, m2_pos, atom2 + 2*orb2);

                        assert(a != b);
                        if (a != b)
                        {
                            Hint_(a, b) = complex<double>(1.0 * U_hoppings[neigh][atom2][atom1], 0.0);
    ...
    for(int atom_l=0;atom_l<n_atoms_;atom_l++){
        ...
        for(m=0;m<nsites_;m++){
            ...
            for(int atom_m=0;atom_m<n_atoms_;atom_m++){
                ...
                Get_minimum_distance_direction(l,atom_l, m,atom_m, r1_,r2_, dis_);
                ...
                if(dis_<=Parameters_.Truncating_Length_in_am){
                    U_val = Parameters_.U0_interatom*( (1.0/(dis_*Parameters_.a_moire))  -
                                                        (1.0/(sqrt( (dis_*dis_*Parameters_.a_moire*Parameters_.a_moire)
                                                                    + Parameters_.d_screening*Parameters_.d_screening)))  )*
...
void Connections_HC_MO::Interactions_Sorting()
{
...
void Connections_HC_MO::Check_up_down_symmetry()
...
void Connections_HC_MO::Check_Hermiticity()
...
void Connections_HC_MO::HTBCreate()
{

//Convention used
//atom=0=Blue  //See Cartoon in notes
//atom=1=Red
...
void Connections_HC_MO::Print_Hopping(){
...
void Connections_HC_MO::Print_LongRangeInt(){
