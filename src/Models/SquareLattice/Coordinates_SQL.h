...
#ifndef Coordinates_SQL_class
#define Coordinates_SQL_class
class Coordinates_SQL {

public:
    Coordinates_SQL(int lx, int ly, int n_orbs)
...
/*  
 * ***********
 *  Functions in Class Coordinates_SQL ------
 *  ***********
*/  

int Coordinates_SQL::indx_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_SQL.h:x-coordinate of lattice excede limit");}
    return indx_basiswise_[i];
    // ----------
}


int Coordinates_SQL::indy_basiswise(int i){
    if(i>nbasis_-1){perror("Coordinates_SQL.h:y-coordinate of lattice excede limit");}
    return indy_basiswise_[i];
    // ----------
}

int Coordinates_SQL::indx_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_SQL.h:x-coordinate of lattice excede limit");}
    return indx_cellwise_[i];
    // ----------
}


int Coordinates_SQL::indy_cellwise(int i){
    if(i>ncells_-1){perror("Coordinates_SQL.h:y-coordinate of lattice excede limit");}
    return indy_cellwise_[i];
    // ----------
}

int Coordinates_SQL::Nbasis(int x, int y, int orb){
    if(!( (x<lx_&& y<ly_) &&  orb<n_orbs_)){perror("Coordinates_SQL.h:ith-sitelabel of lattice excede limit");}
    return Nbasis_[x][y][orb];
    // ----------
}


int Coordinates_SQL::Ncell(int x, int y){
    if(!(x<lx_&& y<ly_)){perror("Coordinates_SQL.h:ith-sitelabel of lattice excede limit");}
    return Ncell_(x,y);
    // ----------
}


int Coordinates_SQL::neigh(int cell, int wneigh){
    if(cell> (lx_*ly_)-1 || wneigh>=8){perror("Coordinates_SQL.h:getneigh -> ifstatement-1");}
    return neigh_(cell,wneigh);
} // ----------


void Coordinates_SQL::Numbering(){
...
int Coordinates_SQL::getneigh(int site,int wneigh){
    if(site>ncells_-1 || wneigh>7){perror("Coordinates_SQL.h:getneigh -> ifstatement-1");}
